    ...
    uint_t flag, znode_t **zpp, zfs_acl_ids_t *acl_ids)
{
	...
	offset_uid = acl_ids->z_fuid >= zfsvfs->z_uid_offset ?
		acl_ids->z_fuid - zfsvfs->z_uid_offset : acl_ids->z_fuid;
	offset_gid = acl_ids->z_fgid >= zfsvfs->z_gid_offset ?
		acl_ids->z_fgid - zfsvfs->z_gid_offset : acl_ids->z_fgid;
...
zfs_rezget(znode_t *zp)
{
	...
	zfs_uid_write(ZTOI(zp), z_uid >= zfsvfs->z_uid_offset ?
            z_uid : z_uid + zfsvfs->z_uid_offset);
	zfs_gid_write(ZTOI(zp), z_gid >= zfsvfs->z_gid_offset ?
            z_gid : z_gid + zfsvfs->z_gid_offset);
